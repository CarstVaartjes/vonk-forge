...
def wheel_path(repository: Path) -> Path:
    path = repository / "inventory/wheels/vonk_agent_protocol-3.0.0-py3-none-any.whl"
...
def test_secret_in_compressed_wheel_member_is_rejected(tmp_path: Path) -> None:
    ...
    assert result.returncode == 1
    assert (
        "inventory/wheels/vonk_agent_protocol-3.0.0-py3-none-any.whl!/" in result.stderr
    )
