...
def test_parses_aggregate_children_and_redacts_logs() -> None:
            ...
            "children": [
                {
                    "kind": "model-cache",
                    "state": "running",
                    "progress": {"phase": "download", "completed_bytes": 80},
                },
                {
                    "kind": "runtime-image",
                    "state": "failed",
                    "progress": {"phase": "build", "completed_bytes": 0},
                    "failure": {
                        "code": "build_failed",
                        "detail": "build step failed",
                        "recovery_actions": ["retry"],
                        "retryable": True,
                        "log_excerpt": "Authorization: Bearer secret",
                    },
                },
            ],
        }
    ...
    assert [member.key for member in operation.members] == [
        "model-cache",
        "runtime-image",
    ]
    assert operation.members[1].failure is not None
    assert (
        operation.members[1].failure.log_excerpt == "Authorization: Bearer <redacted>"
    )
...
def test_exact_failure_fields_include_capacity_and_access_action() -> None:
    ...
    assert (failure.required_bytes, failure.free_bytes, failure.shortfall_bytes) == (
        200,
        100,
        100,
    )
...
def test_resume_selection_is_exact_revision_scoped_and_prefers_active_attempt() -> None:
    ...
    selected = select_availability_operation(
        [value for value in values if value is not None], "revision-a"
    )
    assert selected is not None and selected.id == "active"
    assert (
        select_availability_operation(
            [value for value in values if value is not None], "revision-c"
        )
        is None
    )
