...
class ObservedLifecycle(Protocol):
    def __enter__(self) -> ObservedLifecycle: ...
...
def _configure_acceptance_renewal(
    bundle: Path, *, lifetime_seconds: int
) -> None:
    ...
    try:
        ...
        provisioner = next(
            value
            for value in provisioners
            if value.get("name") == "vonk-forge-agent"
        )
    ...
    environment["VONK_AGENT_CA_CERTIFICATE_LIFETIME_SECONDS"] = str(
        lifetime_seconds
    )
...
class SparkLifecycle:
    def __init__(
        self, arguments: argparse.Namespace, graph: dict[str, object]
    ) -> None:
        ...
        self.control_hostname = (
            f"{CONTROL_HOST_PREFIX}.{self._required_environment('VONK_ACCEPTANCE_TAILNET_DNS_SUFFIX')}"
        )
        ...
        if self.machine not in expected_machine or os.geteuid() == 0:
            raise LifecycleError("Spark lifecycle is not running natively as an ordinary user")
    ...
    def __enter__(self) -> SparkLifecycle:
        try:
    ...
    def _cleanup(self) -> None:
        ...
        if failures:
            raise LifecycleError("isolated Spark lifecycle cleanup failed") from failures[0]
    ...
    def _start_controller(self) -> None:
        ...
        try:
            ...
            raise LifecycleError("candidate controller services are not healthy") from error
    ...
    def _assert_compose_image_graph(self) -> None:
        ...
        for role, service in COMPOSE_IMAGE_ROLES.items():
            ...
            if (
                ...
                or configured_service.get("image") != images.get(role)
            ):
        ...
        for service in services.values():
            ...
            if (
                ...
                or any(value in image for value in (":latest", ":dev", ":main", ":edge"))
            ):
    ...
    def _materialize_synthetic_device(self) -> str:
        ...
        if (
            cdi_target.exists()
            or cdi_target.is_symlink()
            or smi_target.exists()
            or smi_target.is_symlink()
        ):
        ...
        os.chmod(cdi, 0o600)
        os.chmod(smi, 0o700)
        for executable in (Path("/usr/bin/docker"), Path("/usr/bin/nvidia-ctk")):
    ...
    def _create_grant(self) -> tuple[str, str, str, str]:
        ...
        try:
            ...
            raise LifecycleError("single-use enrollment grant creation failed") from error
        expected = {
            "ca_fingerprint",
            "controller_endpoint",
            "enrollment_endpoint",
    ...
    def _wait_for_agent_identity(
        ...
        while time.monotonic() < deadline:
            try:
                ...
                last_error = error
                time.sleep(2)
        raise LifecycleError("direct Rust agent identity did not converge") from last_error
