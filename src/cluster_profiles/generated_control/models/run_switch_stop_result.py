...
from ..models.run_switch_stop_result_subphase_type_0 import check_run_switch_stop_result_subphase_type_0
...
T = TypeVar("T", bound="RunSwitchStopResult")



@_attrs_define
class RunSwitchStopResult:
    """
        Attributes:
            phase (Literal['stop']):
            run_id (str):
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        phase = cast(Literal['stop'] , d.pop("phase"))
        if phase != 'stop':
            raise ValueError(f"phase must match const 'stop', got '{phase}'")
        ...
        def _parse_subphase(data: object) -> Union[None, RunSwitchStopResultSubphaseType0, Unset]:
            ...
            try:
                if not isinstance(data, str):
                    raise TypeError()
                subphase_type_0 = check_run_switch_stop_result_subphase_type_0(data)
        ...
        run_switch_stop_result = cls(
            phase=phase,
            run_id=run_id,
            subphase=subphase,
        )

        return run_switch_stop_result
