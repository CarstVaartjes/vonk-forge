...
Both workloads retain rootless `slirp4netns` isolation. Rank 1 connects to the
exact controller-supplied master destination and lets slirp choose its
container-side source address; it does not attempt to bind the host fabric IP
inside the container namespace. The controller-supplied local fabric address
remains in the bounded HELLO and evidence. Address-specific rank-0 publication,
host routing, and direct-fabric-only firewall evidence prove the physical path.
...
mutable `:dev` Compose graph, generate a fresh 15-file local source generation,
publish the exact two-item project with its 13-file runtime-secret projection,
...
`0700` `secrets/` directory, and exactly 13 mode `0600` regular secret files.
The publication fixture result was `13 passed`.
