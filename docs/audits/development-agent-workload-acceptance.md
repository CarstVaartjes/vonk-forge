...
| Real two-node gang | Run `model-multinode` with both exact target nodes after applying the direct-fabric-only rendezvous firewall rule | Rank-0 address-specific `MASTER_ADDR:29500:29500` publication, slirp-selected container source, positive direct-fabric host probe and negative management/public probes, bounded local-address HELLO/ack, equal image/artifact-set identities, all-rank readiness, sole entrypoint inference | Pending |
| Rank failure/recovery | Stop only the selected rootless workload container while its Rust agent remains healthy; then start that same container | Fresh failed-rank snapshot, route withdrawal, fresh recovery snapshot, republication, recovered inference | Pending |
