...
Use Fleet for the supported, read-only view of enrolled GPU nodes. The control
plane renders the latest authenticated agent observation for each
certificate-bound `spk_…` node. A health read does not install a collector, use
...
co-residency decisions remain admission checks for the selected recipe.
...
## Historical accepted observation on 2026-08-02

A fresh live probe found both nodes `healthy` with no warnings or errors.
Docker was available, each GPU reported 39 C, and both fabric functions on each
node reported speed `200000`, MTU 1500, and RDMA state `ACTIVE`. This is
historical evidence, not a supported operator command to replay.
...
1. Preserve the Fleet node detail JSON from the authenticated API or browser
   evidence export before changing anything.
...
Investigate unexpected changes before recipe activation; never modify the
