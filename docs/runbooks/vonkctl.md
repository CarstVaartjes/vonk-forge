...
vonkctl fleet loginfo Atlas --since 15m --lines 100
```

Friendly Spark names and exact stable selectors are accepted. Mutations are
single-step Controller operations; there is no obsolete `fleet profile`
alias. `detail` retains stale/unsupported measurements as explicit states.
`loginfo` reads bounded sanitized logs through the authenticated Controller
path and never falls back to SSH.
...
the model cache by default; `--with-model` explicitly requests the dependent
model removal. Cached updates are automatically used by the next Profile
load; there is no Profile update command.
...
counts, warnings, and next actions when supplied by the Controller.
