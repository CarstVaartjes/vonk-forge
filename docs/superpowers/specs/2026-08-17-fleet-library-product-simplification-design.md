...
The visible Sparks collection is registration-backed, not repository-
projection-backed:

- A fresh/reset database with no non-revoked enrolled `AgentNode` records
  renders zero Sparks, even if `inventory/fleet.toml` still has projected
  definitions.
...
- Offline or stale enrolled Sparks remain visible with truthful status and
  timestamps.
- Pending, unregistered, and revoked identities do not count as active Sparks.
  Pending evidence remains available in the enrollment flow; revoked identity
  history remains available through the audit trail.
- Repository definitions continue to control allowed metadata, placement, and
  operational validation. They are not themselves shown as Spark cards.

The live Fleet stream and HTTP fleet evidence response use the same rule.
...
- **Manage identity** — inspect certificate state and perform explicit,
  confirmed revoke/re-enrollment actions.
