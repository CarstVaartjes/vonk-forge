...
- [x] **Step 3: Implement the minimal timeout relocation**
...
- [x] **Step 4: Regenerate the canonical recipe context identity**
...
- [x] **Step 5: Verify GREEN and broader contracts**
...
- Retain privately: `.state/development-acceptance/model-multinode-physical-20260813.json`
...
- [x] **Step 1: Run full relevant checks, request independent review, and merge a PR only after required CI is green.**
- [x] **Step 2: Redeploy the accepted NAS `:dev` cohort if its control image changed; preserve all named volumes and secrets.**
- [x] **Step 3: Resume the physical multi-node runner with `--timeout-seconds 1800 --stop-after inference-ok`; require a new source bundle/image identity and no model redownload.**
- [x] **Step 4: Verify firewall/address bindings, stop only rank 1's exact managed container, and resume through `route-withdrawn-after-failure`.**
- [x] **Step 5: Start that same rank-1 container and resume through `inference-recovered`.**
- [x] **Step 6: Restart both supervisors and the NAS project in ordered stop/start form, then resume without `--stop-after` through normal stop, route withdrawal, and uninstall.**
- [x] **Step 7: Update the redacted audit with exact accepted public commit/workflow identities and verified outcomes; never commit private evidence, tokens, keys, model contents, or host-specific secret output.**
