...
| Control worker | Cluster mutation, stale/crashed worker, and internal response spoofing | Production rejects generic job submission without executing an attempt. The worker has a distinct image with no Git/OpenSSH or source mount and no published ports. Its dedicated artifact-egress network permits outbound pinned model and OCI downloads; the optional Hugging Face credential is used only at the canonical provider authority and is stripped from CDN redirects. GPU-node mutations still require Controller-authorized signed operations. It consumes authenticated persisted plans and nonce-bound, short-lived HMAC authority decisions. Transactional locks, leases, attempt fences, and atomic route acknowledgements fence every effect | Worker-starvation/lease alerts and durable operation evidence; fail-closed withdrawal, compensation, or operator review | production-worker, reconciliation, PostgreSQL race, Compose, and built-image boundary tests |
