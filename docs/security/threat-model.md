...
| CLI token | Administrator capability; local unprivileged user | HTTPS origin validation, regular non-symlink token file, bounded JSON, no token in argv/output | API audit and token rotation | `tests/cluster_profiles/test_control_client_requests.py`, `control/tests/test_auth_api.py` |
...
| Control worker | Cluster mutation and stale/crashed workers | The API exposes current recipe, cache, and upgrade operations. The worker has a distinct image with no Git/OpenSSH or source mount and no published ports. Its dedicated artifact-egress network permits outbound model and OCI downloads; the optional Hugging Face credential is used only at the canonical provider authority and is stripped from CDN redirects. GPU-node mutations require Controller-authorized signed operations. Current persisted plans, transactional locks, leases, attempt fences, signed readiness, and atomic route acknowledgements govern execution | Worker-starvation/lease alerts and durable operation evidence; fail-closed withdrawal or operator review | production-worker, recipe-run, agent-upgrade, PostgreSQL race, Compose, and built-image boundary tests |
...
| Spark Docker/NVIDIA runtime | Root-equivalent Docker socket, malicious recipe image, confused retry, arbitrary GPU/host access | The agent cannot read the Docker socket and is never in the Docker group. The API-only Ed25519 authority signs an expiring grant bound to node/job/operation/attempt/fence/action and a canonical request digest. A root helper verifies the local request and imported image receipt, then compiles fixed Docker flags: numeric non-root user, read-only root, init, no launch-time pull, bounded rotating local logs, all capabilities dropped, no-new-privileges, bounded PID/memory/swap/shared-memory, explicitly address-bound declared ports, managed mounts, and the exact optional CDI device `nvidia.com/gpu=all`. Connected multi-node serving has one fixed shape: host networking, private IPC and bounded shared memory, `/dev/infiniband`, exact ulimits, and no Docker publication. The helper validates the requested local/master addresses, rendezvous/endpoint ports against the root-owned firewall configuration, then derives the unique active RoCE v2 HCA/GID and interface from kernel sysfs. The derived environment participates in exact container identity; recipe overrides, missing or ambiguous bindings, and firewall drift reject launch. For that preflight the helper observes host interface addresses through `AF_NETLINK`; read-only iptables policy verification requires `CAP_NET_ADMIN`, while systemd retains `IPAddressDeny=any` and permits no `AF_INET` or `AF_INET6` sockets. Privileged mode, partial or arbitrary host/device/mount variants, wildcard/loopback publications, and socket mounts are rejected | Exact retry uses a canonical container label; conflicting or stopped leftovers fail closed. Stop is idempotent, normal cleanup removes the container, and admission/route publication still require accepted operation evidence | `rust/crates/vonk-agent-helper/tests/authority.rs`, `rust/crates/vonk-agent/tests/workloads.rs`, `control/tests/test_host_helper_authority.py`, `tests/scripts/test_docker_firewall.sh` |
| Recipe build egress | Malicious Dockerfile, SSRF, metadata theft, DNS rebinding, direct bypass of a proxy | Public builds receive only an internal per-build rootless network. A separately packaged static proxy is the sole dual-homed member and admits exact declared hostnames on ports 80/443 after per-connection DNS resolution rejects every non-public answer; it connects the vetted address rather than resolving twice. HTTP methods, headers, concurrency, time, and bytes are bounded. The Controller requires fresh `recipe.build.egress-proxy.v1`; missing binary or old agents fail closed | Sidecar, internal/outbound networks, proxy image, and private graphroot are operation-scoped and removed by normal/error/cancellation cleanup; the systemd control group kills descendants after an agent crash | `rust/crates/vonk-build-egress`, `rust/crates/vonk-agent/tests/recipe_builder.rs`, `control/tests/test_recipe_builds.py` |
...
| GPU node SSH | Break-glass host maintenance; hostile network/node impostor | Never used by the production API/worker or Spark installer. Explicit operator-only maintenance entry points retain strict host keys, no shell interpolation, staged digest checks, and recovery gates | Identity quarantine and console recovery | no-routine-SSH, built-worker-image, install identity/remote/steps tests |
