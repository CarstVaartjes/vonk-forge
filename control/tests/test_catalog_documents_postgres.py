    ...
    with catalog._sessions() as session:
        stored = session.scalar(
            select(CatalogDocumentRevision).where(
                CatalogDocumentRevision.id == active.id
            )
        )
        ...
        assert (
            session.scalar(
                select(CatalogDocumentHead).where(
                    CatalogDocumentHead.active_revision_id == active.id
                )
            )
            is not None
        )
...
def test_recipe_resolution_rejects_wrong_or_missing_exact_model_fk(
    catalog, mutation: str
) -> None:
    model = _model()
...
def test_candidate_switch_is_atomic_and_failed_candidate_preserves_prior_good(
    catalog,
) -> None:
    model = _model()
    model_revision = catalog.create_draft(model, actor="operator")
...
def test_recipe_reuse_keys_follow_effective_execution_and_model_artifacts(
    catalog,
) -> None:
    original = _model()
    ...
    _selected_model_reference(changed_recipe)["content_sha256"] = (
        changed_model_revision.content_digest
    )
    successor = catalog.revise(
        first_recipe.document_id, changed_recipe, actor="operator", expected_revision=1
    )
    catalog.resolve(successor.id, actor="operator", expected_revision=2)
