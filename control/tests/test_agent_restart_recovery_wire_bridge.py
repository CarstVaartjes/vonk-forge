    ...
    with sessions.begin() as session:
        for node_id in (NODE_A, NODE_B):
            session.add(AgentNode(
                node_id=node_id, state="active", capabilities=[],
...
def _assignment(source: MemoryVerifiedObjectSource, now: datetime):
    ...
    assignment = DistributionAssignment.parse({
        ...
        "objects": [
            {"name": "weights/shard.bin", "sha256": large_digest, "bytes": len(large), "kind": "model"},
            {"name": "config/tokenizer.json", "sha256": small_digest, "bytes": len(small), "kind": "model"},
            {"name": "image.oci.tar", "sha256": archive_digest, "bytes": len(archive), "kind": "oci-archive"},
        ],
...
class DistributionHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if target.path.startswith("/agent/distribution/manifests/"):
            ...
            return
        if not target.path.startswith("/agent/distribution/objects/") or plan_digest is None:
            self.send_error(404)
            return
        ...
        self.end_headers()
        if digest == server.large_digest and start == 0 and not server.partial_sent.is_set():
...
def distribution_https(tmp_path: Path, controller):
    ...
    assignment, large_digest, small_digest, archive_digest = _assignment(source, clock.now)
...
def restart_probe() -> Path:
    ...
    if path is None:
        ...
        subprocess.run([
            "cargo", "build", "--locked", "-p", "vonk-agent",
            "--example", "restart_recovery_probe",
        ], cwd=root, check=True)
        path = str(Path(os.environ.get("CARGO_TARGET_DIR", root / "target")) / "debug/examples/restart_recovery_probe")
    probe = Path(path)
    assert probe.is_file() and os.access(probe, os.X_OK)
    return probe


def _probe_request(mode: str, claim, root: Path, server, certs: dict[str, Path]) -> dict[str, object]:
    return {
        "mode": mode,
        "data_root": str(root),
        ...
        **{key: str(certs[key]) for key in (
            "ca_pem", "certificate_pem", "chain_pem", "private_key_pem"
        )},
    }
...
def _run_probe(probe: Path, document: dict[str, object]) -> AgentResult:
    completed = subprocess.run(
        [str(probe)], input=json.dumps(document), text=True,
...
def test_dead_agent_resumes_partial_transfer_from_fresh_controller_claim(
    ...
    controller, distribution_https, restart_probe: Path, tmp_path: Path
) -> None:
    sessions, clock = controller
    ...
    operation = jobs.enqueue(
        parent_job.id, NODE_A, "artifact.distribution.v1", assignment.plan_digest,
    ...
    first = claim_agent(jobs, NODE_A, "serial-a", 30, runtime_identity=PACKAGED_RUNTIME_IDENTITY)
    assert first is not None and first.operation_id == operation.id
    data_root = tmp_path / "agent-data"
    first_process = subprocess.Popen(
        [str(restart_probe)], stdin=subprocess.PIPE, stdout=subprocess.PIPE,
        stderr=subprocess.PIPE, text=True,
    )
    assert first_process.stdin is not None
    first_process.stdin.write(json.dumps(_probe_request(
        "execute-distribution", first, data_root, server, certs
    )))
    ...
    try:
        ...
        while time.monotonic() < deadline:
            if server.partial_sent.is_set() and partial.exists() and partial.stat().st_size > 0 and small.exists():
                break
            if first_process.poll() is not None:
                raise AssertionError(first_process.stderr.read() if first_process.stderr else "probe exited")
            time.sleep(0.05)
        else:
            raise AssertionError("agent did not persist partial bytes and a completed object")
    ...
    interrupted = _run_probe(restart_probe, _probe_request(
        "recover", first, data_root, server, certs
    ))
    ...
    second = claim_agent(jobs, NODE_A, "serial-a", 30, runtime_identity=PACKAGED_RUNTIME_IDENTITY)
    ...
    completed = _run_probe(restart_probe, _probe_request(
        "execute-distribution", second, data_root, server, certs
    ))
    ...
    assert any(digest == large_digest and start == saved_bytes for digest, start, _ in requests)
    assert sum(digest == small_digest for digest, _, _ in requests) == 1
    assert sum(digest == archive_digest for digest, _, _ in requests) == 1
    assert hashlib.sha256((data_root / "distribution/models" / large_digest).read_bytes()).hexdigest() == large_digest
