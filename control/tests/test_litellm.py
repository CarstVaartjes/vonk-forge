...
def _snapshot():
    return RouteState(
        ...
        {"deepseek": "http://node.internal:8000/v1"},
...
def test_apply_is_atomic_and_retains_previous_generation(tmp_path: Path) -> None:
    ...
    publisher = LiteLlmPublisher(
        tmp_path,
        validate=lambda content: b"deepseek" in content,
        apply=lambda content: applied.append(content),
    )
    generation = publisher.publish(_snapshot(), _policy())
    assert publisher.active() == generation
    rejecting = LiteLlmPublisher(
        tmp_path, validate=lambda _: False, apply=lambda _: None
    )
...
def test_hermes_group_is_local_ordered_and_retry_bounded(tmp_path: Path) -> None:
    ...
    hermes = [
        model for model in config["model_list"] if model["model_name"] == "hermes-agent"
    ]
...
@pytest.mark.parametrize(
    ...
    (
        LiteLlmDeployment(
            "hermes-agent", "cloud", "https://api.openai.com/v1", 1, 1, 1
        ),
