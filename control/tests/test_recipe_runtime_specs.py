...
def _json_object(value: object) -> dict[str, object]:
    """Narrow decoded JSON to a JSON object; a wrong shape fails the test."""
...
def _json_array(value: object) -> list[object]:
    """Narrow decoded JSON to a JSON array; a wrong shape fails the test."""
...
def _distributed_sglang_recipe() -> RecipeDefinition:
    ...
    runtime["arguments"] = [{"name": "model-path", "value": "/models"}]
    model_entry = _json_object(_json_array(raw["models"])[0])
    _json_object(_json_array(model_entry["files"])[0])["roles"] = [
        "entrypoint",
        "worker",
    ]
    topology = _json_object(raw["topology"])
    endpoint_role = copy.deepcopy(_json_object(_json_array(topology["roles"])[0]))
...
def _multi_artifact_inputs() -> tuple[RecipeDefinition, ModelDefinition]:
    ...
    runtime = _json_object(raw["runtime"])
    runtime["entrypoint"] = ["/opt/vonk/bin/vllm", "serve", "/models/target"]
    _json_array(runtime["arguments"]).append(
        {
...
def test_runtime_spec_writable_paths_ignore_recipe_metadata_identity() -> None:
    ...
    values = {
        ...
        for item in _json_array(runtime["environment"])
    }
    ...
    assert any(
        _json_object(item)["path"] == "/outputs/cache/vllm"
        for item in _json_array(runtime["writable_paths"])
    )
...
def test_runtime_spec_projects_distributed_sglang_placement_authority() -> None:
    ...
    assert entry_runtime["placement_environment"] == {
        "local_address": "VONK_LOCAL_ADDR",
        "master_address": "VONK_MASTER_ADDR",
        "master_port": "VONK_MASTER_PORT",
    }
    entry_command = _json_array(entry_runtime["entrypoint"])
