...
def test_publication_lock_claim_is_bounded_when_another_publisher_holds_it(
    ...
    so a second descriptor in this process would succeed and never exercise
    the contended path. It fails on the wrong implementation that calls a
    blocking ``flock``, because that call never returns while the holder lives.
    """
    ...
    # With the holder gone the same call succeeds.
