...
def observation_id(boundary: str) -> str:
    return str(
        uuid.uuid5(uuid.NAMESPACE_URL, f"vonk-forge:deployment-observation:{boundary}")
    )
...
def stored_observation(session, boundary: str) -> PlatformObservation | None:
    ...
    return (
        PlatformObservation.model_validate_json(json.dumps(row.payload))
        if row is not None
        else None
    )
...
class DeploymentObserver:
    def __init__(self, sessions, *, channel="dev", transport=None, clock=None):
        ...
        self.pool = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="deployment-observer"
        )
        ...
        for boundary, read in (
            ...
            try:
                ...
                with self.sessions.begin() as session:
                    ...
                    if row is None:
                        row = Observation(
                            id=observation_id(boundary),
                            node_id="controller",
                            kind=f"deployment.{boundary}",
                        )
                        session.add(row)
    ...
    def repository(self) -> PlatformObservation:
        response = self.client.get(
            "https://api.github.com/repos/CarstVaartjes/vonk-forge/commits/main",
            headers={"Accept": "application/vnd.github+json"},
        )
        response.raise_for_status()
        head = RepositoryHead.model_validate_json(response.content)
        return PlatformObservation(
            source="GitHub repository main (HTTPS observation)",
    ...
    def publication(self) -> PlatformObservation:
        ...
        response = self.client.get(
            f"{origin}/artifacts/{self.channel}/current.manifest"
        )
        response.raise_for_status()
        lines = [line.split("=", 1) for line in response.text.splitlines() if line]
        pointer = dict(lines)
        if (
            len(pointer) != len(lines)
            or pointer.get("schema_version") != "2"
            or pointer.get("channel") != self.channel
        ):
        ...
        if (
            ...
            or document.channel != self.channel
            or document.source_sha != pointer["source_sha"]
        ):
            raise ValueError("publication identities disagree")
        image = document.images["api"].rsplit("@", 1)[-1]
        return PlatformObservation(
            source="Public installer channel (HTTPS observation)",
            observed_at=self.clock(),
            source_commit=document.source_sha,
            image_digest=image,
            manifest_sha256=hashlib.sha256(response.content).hexdigest(),
        )
