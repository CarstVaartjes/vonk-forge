...
class LifecycleService(Protocol):
    ...
    def install(self, plan: Any, *, plan_digest: str, actor: str, request_id: str) -> Any: ...
    def preview_run(self, installation_id: str, alias: str) -> Any: ...
    def start(self, plan: Any, *, plan_digest: str, actor: str, request_id: str) -> Any: ...
    def preview_stop(self, run_id: str) -> Any: ...
    def stop(self, run_id: str, *, plan_digest: str, actor: str, request_id: str) -> Any: ...
    def preview_uninstall(self, installation_id: str) -> Any: ...
    def uninstall(self, installation_id: str, *, plan_digest: str, actor: str, request_id: str) -> Any: ...
...
class LibraryLifecycleAuthority:
    ...
    @staticmethod
    def require_operate(actor: Actor | str, capabilities: Sequence[str] | None = None) -> None:
    ...
    def preview_placement(
        ...
        *,
        ...
        eligible = tuple(sorted(
            spark.node_id for spark in sparks
            if spark.online and spark.free_bytes >= required_bytes
            and spark.free_memory_bytes >= required_memory_bytes
            and required <= set(spark.capabilities)
        ))
        rejected = {
            spark.node_id: "offline" if not spark.online else (
                "missing capability" if not required <= set(spark.capabilities)
                else "insufficient capacity"
            )
            for spark in sparks if spark.node_id not in eligible
        }
        if len(eligible) < count:
            raise LibraryPlacementConflict("fewer eligible Sparks than topology requires")
        ...
        return PlacementPreview(
            topology=dict(topology), nodes=nodes, eligible=eligible,
            rejected=rejected, required_bytes=required_bytes,
            required_memory_bytes=required_memory_bytes, placement_digest=digest,
        )
    ...
    def install(self, plan: Any, *, plan_digest: str, actor: Actor, request_id: str, capabilities: Sequence[str] = ()) -> Any:
        self.require_operate(actor, capabilities)
        return self.service.install(plan, plan_digest=plan_digest, actor=actor.subject, request_id=request_id)

    def run(self, plan: Any, *, plan_digest: str, actor: Actor, request_id: str, capabilities: Sequence[str] = ()) -> Any:
        self.require_operate(actor, capabilities)
        return self.service.start(plan, plan_digest=plan_digest, actor=actor.subject, request_id=request_id)

    def stop(self, run_id: str, *, plan_digest: str, actor: Actor, request_id: str, capabilities: Sequence[str] = ()) -> Any:
        self.require_operate(actor, capabilities)
        return self.service.stop(run_id, plan_digest=plan_digest, actor=actor.subject, request_id=request_id)

    def uninstall(self, installation_id: str, *, plan_digest: str, actor: Actor, request_id: str, capabilities: Sequence[str] = ()) -> Any:
        self.require_operate(actor, capabilities)
        return self.service.uninstall(installation_id, plan_digest=plan_digest, actor=actor.subject, request_id=request_id)

    def operation(self, operation_id: str, *, actor: Actor | None = None, capabilities: Sequence[str] = ()) -> Any:
        if actor is not None:
            self.require_operate(actor, capabilities)
        return self.service.get(operation_id)

    def retry(self, operation_id: str, *, actor: Actor, request_id: str, capabilities: Sequence[str] = ()) -> Any:
        self.require_operate(actor, capabilities)
        return self.service.retry(operation_id, actor=actor.subject, request_id=request_id)


__all__ = [
    "LibraryLifecycleAuthority", "LibraryOperationDenied", "LibraryPlacementConflict",
    "PlacementPreview", "SparkCapacity",
]
