...
class LibraryRecipeService:
    ...
    def save(self, *, slug: str, content: Mapping[str, object], actor: Any, source: str = "custom") -> LibraryRecipeRevision:
        ...
        title = _text(document.get("title") or document.get("metadata", {}).get("title"), "title")
        with self._sessions.begin() as session:
            if session.scalar(select(Recipe).where(Recipe.slug == slug)) is not None:
                raise LibraryRecipeConflict("library.recipe_exists", "recipe slug already exists")
            recipe = Recipe(recipe_id=recipe_id, slug=slug, title=title, source=source,
                            created_by=self._actor(actor), created_at=now, updated_at=now)
            revision = RecipeRevision(revision_id=revision_id, recipe_id=recipe_id, revision_number=1,
                                      content=copy.deepcopy(dict(document)), content_digest=digest,
                                      created_by=self._actor(actor), created_at=now)
            session.add_all((recipe, revision))
        return self._view(recipe, revision)

    def edit(self, recipe_id: str, content: Mapping[str, object], actor: Any) -> LibraryRecipeRevision:
        self._require_write(actor)
        with self._sessions.begin() as session:
            recipe = session.scalar(select(Recipe).where(Recipe.recipe_id == recipe_id).with_for_update())
            if recipe is None:
                raise KeyError(recipe_id)
            document = self._validate(recipe.slug, content)
            digest = recipe_content_sha256(document)
            latest = session.scalar(select(RecipeRevision).where(RecipeRevision.recipe_id == recipe_id).order_by(RecipeRevision.revision_number.desc()).limit(1))
            assert latest is not None
            if latest.content_digest == digest:
                raise LibraryRecipeConflict("library.duplicate_revision", "recipe content is unchanged")
            now = _aware(self._clock())
            revision = RecipeRevision(revision_id=str(uuid.uuid4()), recipe_id=recipe_id,
                                      revision_number=latest.revision_number + 1,
                                      content=copy.deepcopy(dict(document)), content_digest=digest,
                                      created_by=self._actor(actor), created_at=now)
            recipe.title = _text(document.get("title") or document.get("metadata", {}).get("title"), "title")
    ...
    def get(self, recipe_id: str, revision_number: int | None = None) -> LibraryRecipeRevision:
    ...
    def list(self) -> list[LibraryRecipeRevision]:
        with self._sessions() as session:
            rows = session.execute(select(Recipe, RecipeRevision).join(RecipeRevision, RecipeRevision.recipe_id == Recipe.recipe_id).order_by(Recipe.slug, RecipeRevision.revision_number)).all()
    ...
    def _validate(slug: str, content: Mapping[str, object]) -> dict[str, object]:
        if not isinstance(slug, str) or _SLUG.fullmatch(slug) is None:
            raise LibraryRecipeError("library.invalid_slug", "slug must be lowercase alphanumeric-hyphen text")
        if not isinstance(content, Mapping):
            raise LibraryRecipeError("library.invalid_content", "recipe content must be an object")
    ...
    def _view(recipe: Recipe, revision: RecipeRevision) -> LibraryRecipeRevision:
        return LibraryRecipeRevision(revision.revision_id, recipe.recipe_id, recipe.slug, recipe.title,
                                     recipe.source, revision.revision_number, copy.deepcopy(revision.content),
                                     revision.content_digest, revision.created_by, revision.created_at)


def _text(value: object, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise LibraryRecipeError("library.invalid_recipe", f"recipe {field} is required")
    return value.strip()
...
__all__ = ["LibraryRecipeError", "LibraryRecipeConflict", "LibraryRecipeRevision", "LibraryRecipeService"]
