...
def load_package_source(
    ...
    response = client.get(
        f"/artifacts/{channel}/agent-builds/{build_digest[7:]}/{binary_digest}/package.json"
    )
    ...
    if (
        source.build_digest != build_digest
        or source.package.binary_sha256 != binary_digest
    ):
