    ...
    def builder(
        ...
        *,
        ...
        if operation.state != "succeeded":
            ...
            retryable = (
                classify(kind_for_agent_error(failure_document))
                is RecoveryDecision.RETRY
            )
            summary = (
                failure.summary
                or failure.reason
                or "canonical Recipe build failed"
            )
            category = (
                failure.diagnostics.category
            ...
            raise RecipeImageAvailabilityError(
                ...
                retryable=retryable,
                retry_after_seconds=(
                    failure.retry_after_seconds if retryable else None
                ),
        ...
        if not isinstance(operation.result, Mapping):
            raise RecipeImageAvailabilityError(
                "recipe_image.build_invalid",
                "canonical Recipe build result is invalid",
                step="build",
            )
        aggregate = operation.result
