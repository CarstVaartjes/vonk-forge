...
for (const width of [1440, 390]) {
  test(`Fleet shows measured lifecycle phase at ${width}px`, async ({page}, testInfo) => {
    ...
    await page.route("**/api/fleet-profiles", route => route.fulfill({json: {schema_version: 2, profiles: [profile]}}));
    await page.route("**/api/fleet-profiles/*/preview", route => route.fulfill({json: {schema_version: 2, profile_id: id, profile_name: "Studio", plan_digest: "b".repeat(64), allowed: true, steps: [{index: 0, kind: "start", label: "Run Qwen Chat", node_ids: [nodeId]}], reasons: []}}));
    await page.route("**/api/fleet-profiles/*/apply", route => route.fulfill({json: application}));
    await page.route("**/api/fleet-profile-applications/*", route => route.fulfill({json: application}));
