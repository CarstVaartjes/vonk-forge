...
export function libraryRecipeSummary(input: Partial<LibraryRecipeSummary> & Pick<LibraryRecipeSummary, "recipe_id" | "slug" | "title">): LibraryRecipeSummary {
  return {
    ...
    profiles: [{name: "pair", description: "Two connected nodes", node_count: 2, roles: ["leader", "worker"], fabric_connectivity: "connected", minimum_bandwidth_mbps: 10_000}],
    reasons: [],
...
export const unlinkedRecipe = libraryRecipeSummary({recipe_id: "recipe-unlinked", slug: "custom", title: "Custom Runtime", selected_revision: null, profiles: [], capabilities: []});
...
export const fullLibraryDetail: LibraryRecipeDetail = {
  ...
  visual_recipe: {
    ...
    workload: {family: "qwen/3", capabilities: ["openai.chat", "tools"]},
    build: {context: {sha256: "b".repeat(64), expected_bytes: 4096, media_type: "application/vnd.vonk-forge.source-bundle.v1+tar"}, dockerfile: "Dockerfile", download_bytes: 60 * GIB, memory_bytes: 8 * GIB, network_hosts: [], network_mode: "none", platform: "linux/arm64", temporary_bytes: 12 * GIB, timeout_seconds: 3600},
    artifacts: [{id: "weights", kind: "huggingface.snapshot", repository: "Qwen/Qwen3", revision: "c".repeat(40), download_bytes: 50 * GIB, installed_bytes: 52 * GIB, roles: ["leader", "worker"]}],
    runtime: {adapter: "vllm", adapter_version: 1, endpoint_port: 8000, endpoint_protocol: "openai", health_path: "/v1/models", interface: "vonk.runtime.v1", model_aliases: ["qwen-chat"]},
  ...
  profiles: [{
    name: "pair", description: "Two connected nodes", node_count: 2, strategy: "tensor_parallel", measurement: "declared",
    parallelism: {tensor: 2, pipeline: 1, data: 1, backend: "nccl"}, fabric: {connectivity: "connected", minimum_bandwidth_mbps: 10_000}, parameter_overrides: {},
    roles: [
  ...
  operational_state: {
    ...
    mappings: [{mapping_id: "mapping-chat", recipe_revision_id: "revision-chat", profile_name: "pair", generation: 4, state: "ready", nodes: [{node_id: "node-alpha", rank: 0, role: "leader", endpoint_owner: true}, {node_id: "node-beta", rank: 1, role: "worker", endpoint_owner: false}]}],
  ...
  placement: [{
    profile_name: "pair", node_count: 2, candidate_node_ids: ["node-alpha", "node-beta", "node-gamma", "node-delta"], evaluated_group_count: 512,
