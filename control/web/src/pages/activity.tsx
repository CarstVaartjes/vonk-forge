...
import type {AuditSummary, ControlApi, JobSummary} from "../api/types";
    ...
    {events.map((event, index) => {
      ...
      return <li key={`${event.request_id}:${event.action}:${index}`} className={`activity-event is-${status}`}>
        ...
        <article>
          ...
          <div className="activity-event-meta"><span>By <strong>{event.actor || "Unknown operator"}</strong></span><EventTime event={event} now={now}/></div>
  ...
  return <div className="activity-table-wrap"><table className="activity-table">
    ...
    <tbody>{events.map((event, index) => {
      ...
      return <tr key={`${event.request_id}:${event.action}:${index}`}>
        <td data-label="Event"><strong>{activityActionLabel(event.action)}</strong><small>{activityCategory(event)}</small></td>
...
function ActivityOverview({events}: {events: ActivityRecord[]}) {
  ...
  return <section className="activity-overview" aria-label="Activity summary">
...
export function ActivityPage({api, now = new Date()}: {api: Pick<ControlApi, "audit" | "jobs">; now?: Date}) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    void Promise.all([api.audit(), api.jobs()]).then(([audit, operations]) => {
      if (active) setEvents([
        ...audit.events.map(event => ({...event, source: "audit" as const})),
        ...operations.jobs.map(job => operationRecord(job as TimestampedJob)),
  ...
  return <div className="activity-page">
    <header className="activity-hero">
      ...
      <button type="button" className="button secondary" disabled={loading} onClick={() => setAttempt(value => value + 1)}>{loading && events ? "Refreshing…" : "Refresh activity"}</button>
    </header>
    ...
    <section className="activity-controls" aria-label="Activity controls">
      ...
      <div className="activity-control-footer">
        <span role="status">{events ? `${filtered.length} of ${events.length} ${events.length === 1 ? "event" : "events"}` : "Loading activity"}</span>
    ...
    {!loading && !error && events?.length === 0 && <section className="activity-state"><strong>No activity yet</strong><p>Meaningful changes will appear here as the Forge is used.</p></section>}
