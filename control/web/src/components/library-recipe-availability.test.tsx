...
test("starts one aggregate operation and uses the explicit Model access resume route", async () => {
  ...
  const checkAccess = vi.fn(async () => ({schema_version: 2, id: "model-child-1", request_key: "request-2", kind: "download", state: "queued", attempt: 2, artifact_set_sha256: "b".repeat(64), plan_digest: "c".repeat(64), progress: {schema_version: 2, phase: "queued", completed_artifacts: 0, total_artifacts: 1, downloaded_bytes: 0, expected_bytes: 50, total_bytes_known: true, current_artifact_key: null, measurement: {phase: "queued", completed_bytes: 0, total_bytes: 50, total_bytes_known: true, completed_items: 0, total_items: 1}}, failure: null, result: null, created_at: "2026-09-06T12:00:00Z", updated_at: "2026-09-06T12:01:00Z", completed_at: null}));
