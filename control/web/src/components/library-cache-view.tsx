...
export function LibraryModelDownloadAction({api, model, onComplete}: {api: ControlApi; model: LibraryModel; onComplete?(): void}) {
  ...
  const retryable = operation?.state === "failed" && operation.result?.retryable === true;
