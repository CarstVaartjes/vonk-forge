...
const sourceGroup = {
  ...
  topology_name: "solo",
  node_ids: [nodeA],
  nodes: [{node_id: nodeA, rank: 0, role: "leader", endpoint_owner: true}],
...
const canonicalDetail = {
  ...
  recipe: {recipe_id: "recipe-chat", publisher: "local", slug: "qwen-chat", title: "Qwen Chat", description: "Fast distributed chat model.", content_sha256: recipeRevisionId},
  definition: {identity: {publisher: "local", slug: "qwen-chat"}, metadata: {title: "Qwen Chat", description: "Fast distributed chat model.", tags: []}, topology: {name: "solo", mode: "single", node_count: 1, fabric: {connectivity: "none", minimum_bandwidth_mbps: 0}, parallelism: {backend: "local", world_size: 1, tensor: 1, pipeline: 1, data: 1}, roles: [{name: "leader", count: 1, endpoint_owner: true, resources: {disk: {image_bytes: 1, artifact_bytes: 1, staging_bytes: 1, cache_bytes: 0, rollback_bytes: 0, safety_margin_bytes: 1}, memory: {kind: "host", startup_peak_bytes: 1, steady_state_bytes: 1, runtime_growth_bytes: 0, system_reserve_bytes: 1}}}], start_order: ["leader"], stop_order: ["leader"]}},
...
test("one Run action previews and applies the exact model and selected Spark group", async () => {
  ...
  render(<LibraryRecipeAuthority api={{} as never} nodeNames={{[nodeA]: "Spark One"}} runApi={{previewRecipeRunSwitch, applyRecipeRunSwitch, getRecipeRunSwitchOperation: vi.fn(async () => started)}} detail={detail as unknown as LibraryRecipeDetail} catalogRecipe={undefined} modelVersionSha256={digest} onRefresh={async () => undefined} policy={{inventory_fresh_seconds: 300, telemetry_live_seconds: 6, telemetry_delayed_seconds: 20} as never}/>);
  await act(async () => {
