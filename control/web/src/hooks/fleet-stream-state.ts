...
import {telemetryFreshnessAt} from "../lib/fleet";
...
export function fleetStreamReducer(state: FleetStreamState, action: FleetStreamAction): FleetStreamState {
  switch (action.type) {
    case "requested-snapshot":
      ...
      return {...state, error: "", loading: false, snapshot: action.snapshot};
    case "reset-snapshot":
      return {
        ...state,
        error: "",
    ...
    case "node-telemetry": {
      if (!state.snapshot || action.cursor <= state.snapshot.event_cursor) return state;
    ...
    case "projection-refresh":
      ...
      return {
        ...
        snapshot: {...state.snapshot, event_cursor: action.cursor},
      };
    case "stream-open":
