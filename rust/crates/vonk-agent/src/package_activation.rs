...
pub async fn acknowledge(
...
) -> Result<Option<PackageActivationReceipt>, AgentUpgradeError> {
    ...
    let acknowledged =
        read_receipt(Path::new(RECEIPT))?.ok_or(AgentUpgradeError::HelperResponseInvalid)?;
    if acknowledged.phase != PackageActivationPhase::Acknowledged
        || acknowledged.attempt_nonce != receipt.attempt_nonce
        || acknowledged.candidate_package_sha256 != receipt.candidate_package_sha256
    {
        return Err(AgentUpgradeError::HelperResponseInvalid);
    }
