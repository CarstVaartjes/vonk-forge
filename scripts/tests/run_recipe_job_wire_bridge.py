#!/usr/bin/env python3
"""Build the Rust recipe-job wire probe and run its Controller bridge test."""
...
def main() -> int:
    ...
    if not probe.is_file() or not os.access(probe, os.X_OK):
        raise SystemExit(f"recipe-job wire probe is not executable: {probe}")
    environment = os.environ.copy()
    environment["VONK_RECIPE_JOB_WIRE_PROBE"] = str(probe.resolve())
    ...
    return subprocess.run(
        [
            ...
            "-m",
            "pytest",
            "control/tests/test_recipe_job_wire_bridge.py",
            *pytest_args,
        ],
        cwd=repository,
        env=environment,
