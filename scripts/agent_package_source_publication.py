...
def package_source(path: Path, *, version: str) -> AgentPackageSource:
    ...
    subjects = {
        item["name"]: item["digest"]["sha256"] for item in provenance["subject"]
    }
    with tempfile.TemporaryDirectory(prefix="package-source-") as temporary:
        subprocess.run(
            ["/usr/bin/dpkg-deb", "--extract", str(path), temporary],
            check=True,
            capture_output=True,
            timeout=60,
        )
        ...
        binary = hashlib.sha256(
            (extracted / "usr/lib/vonk-forge/vonk-agent").read_bytes()
        ).hexdigest()
        helper = hashlib.sha256(
            (extracted / "usr/lib/vonk-forge/vonk-agent-helper").read_bytes()
        ).hexdigest()
        key = Ed25519PublicKey.from_public_bytes(
            bytes.fromhex(
                (extracted / "usr/share/keyrings/vonk-forge-release.pub")
                .read_text()
                .strip()
            )
        )
        key.verify(
            ...
            b"VONK-HOST-ARTIFACT-V1\x00deb\x00" + hashlib.sha256(raw).digest(),
        )
    if (
        subjects.get("vonk-agent") != binary
        or subjects.get("vonk-agent-helper") != helper
    ):
    ...
    return AgentPackageSource(
        ...
        build_digest=provenance["predicate"]["buildDefinition"]["externalParameters"][
            "build_digest"
        ],
        ...
        package_url=f"https://install.vonkforge.ai/artifacts/agent-packages/{digest}/vonk-forge-agent.deb",
    )
