...
def _distributed_start(*, phase: str | None = None, collective: bool = False) -> dict[str, Any]:
    plan = copy.deepcopy(PLAN)
    plan["topology"].update(world_size=2, node_count=2, rank=1, role="worker", mode="distributed")
...
def _rust_accepts(probe: Path, operation: AgentOperation, payload: dict[str, Any]) -> bool:
...
def _schema_for_value(root: dict[str, Any], schema: dict[str, Any], value: Any) -> dict[str, Any]:
    if "$ref" in schema:
        target = root
    ...
    for candidate in schema.get("anyOf", []):
        ...
        matches = (
            ...
            or (isinstance(value, int) and not isinstance(value, bool) and candidate_type == "integer")
...
def _object_paths(
    root: dict[str, Any], schema: dict[str, Any], value: Any, path: tuple[str | int, ...] = ()
...
def _with_unknown(document: dict[str, Any], path: tuple[str | int, ...]) -> dict[str, Any]:
...
def _plans_and_models() -> Iterator[tuple[AgentOperation, type[BaseModel], dict[str, Any]]]:
    ...
    yield AgentOperation.RECIPE_START, RecipeStartPayload, _distributed_start(phase="rank-launch")
    yield AgentOperation.RECIPE_START, RecipeStartPayload, _distributed_start(
        phase="collective-readiness", collective=True
    )
...
def test_canonical_pydantic_schema_is_exercised_by_rust_serde(
    ...
    for operation, model, payload in _plans_and_models():
        ...
        for path, object_schema in _object_paths(schema, schema, payload):
            ...
            assert object_schema.get("additionalProperties") is not True
    assert (AgentOperation.RECIPE_INSTALL, ("compiled_execution_plan", "runtime", "placement")) in checked_paths
    assert (AgentOperation.RECIPE_INSTALL, ("compiled_execution_plan", "runtime_image", "distribution_object")) in checked_paths
    assert (AgentOperation.RECIPE_INSTALL, ("compiled_execution_plan", "job")) in checked_paths
    assert (AgentOperation.RECIPE_START, ("compiled_execution_plan", "endpoint")) in checked_paths
        ...
        for path, object_schema in _object_paths(schema, schema, payload):
            for name, field_schema in object_schema.get("properties", {}).items():
                if name not in _at(payload, path):
                    continue
                resolved = _schema_for_value(schema, field_schema, _at(payload, (*path, name)))
                ...
                if resolved.get("type") == "string":
                    ...
                    invalid = {}
                elif resolved.get("type") == "object" and resolved.get("additionalProperties") is not True:
                    invalid = []
                else:
                    continue
